...
function playAudio() {
    ...
    source.start();
    // フェードイン
    gainNode.gain.linearRampToValueAtTime(Number(volumeBar.value) / 100, audioCtx.currentTime + 2);
    // ループ設定
    source.loop = true;
}
function stopAudio() {
...
volumeBar.addEventListener("input", () => {
    if (gainNode) {
        gainNode.gain.setValueAtTime(Number(volumeBar.value) / 100, audioCtx.currentTime);
    }
